...
bool Map::numbers(int pixNum , QVector<QPair<int,int>> tile_nums){
    if((pixNum <= 2 && tile_nums[pixNum].second == 4 ) || (pixNum >= 3 && tile_nums[pixNum].second == 3))
        return true;
    return false;
}
/* make a random board */
void Map::randomMap(QVector<QPixmap> pixs,QVector<QPixmap> pixs_nums){
    ...
    int pixNum;
    int tokenNum;
    QVector<QPair<int,int>> tile_nums{{0,0},{1,0},{2,0},{3,0},{4,0}};
    for(int i=0;i<19;i++){
        if(i!=15){
            tokenNum = rand() % 10;
            ...
            while(numbers(pixNum , tile_nums)){
                pixNum = rand()%5;
            }

            tile_nums[pixNum].second++;
            pix = pixs[pixNum];
    ...
    ui(new Ui::Map)
{
    ...
    QPixmap pixmap_num2("/Users/macbook/Desktop/AP_project/num_2.jpg");
    QPixmap pixmap_num3("/Users/macbook/Desktop/AP_project/num_3.jpg");
    QPixmap pixmap_num4("/Users/macbook/Desktop/AP_project/num_4.jpg");
    QPixmap pixmap_num5("/Users/macbook/Desktop/AP_project/num_5.jpg");
    QPixmap pixmap_num6("/Users/macbook/Desktop/AP_project/num_6.jpg");
    QPixmap pixmap_num8("/Users/macbook/Desktop/AP_project/num_8.jpg");
    QPixmap pixmap_num9("/Users/macbook/Desktop/AP_project/num_9.jpg");
    QPixmap pixmap_num10("/Users/macbook/Desktop/AP_project/num_10.jpg");
    QPixmap pixmap_num11("/Users/macbook/Desktop/AP_project/num_11.jpg");
    QPixmap pixmap_num12("/Users/macbook/Desktop/AP_project/num_12.jpg");
    ...
    for(int i=0;i<t.size();i++){
        ...
        connect(t[i],SIGNAL(clicked()),signalMapper,SLOT(map()));
        signalMapper->setMapping(t[i],t[i]);
        connect(signalMapper, SIGNAL(mappedObject(QObject*)),this, SLOT(getAdjacentTile(QObject*)));
    }

    for(int i=0;i<tokens.size();i++){
        signalMapper = new QSignalMapper(this);
        connect(tokens[i],SIGNAL(clicked()),signalMapper,SLOT(map()));
        signalMapper->setMapping(tokens[i],i);
        connect(signalMapper, SIGNAL(mappedInt(int)),this, SLOT(moveRobber(int)));
    }
...
/* settlement's adjacent */
QVector<QPair<tileType,int>> Map::getAdjacentVertex(QPushButton* p){
    QVector<QPair<tileType,int>> tileAdj;
    for(int i=0;i<tiles.size();i++){
        if(abs((tiles[i]->x()+60) - p->x() ) < 85 && abs((tiles[i]->y()+70) - p->y() ) < 85 ){
            tileAdj.append(qMakePair(tiles[i]->getType(),tiles[i]->getToken()));
            qDebug() << "asddfasdfghjkl   " << tiles[i] ;
        }
    }
    return tileAdj;
}
...
/* tile's adjacent */
QVector<QPair<int,int>> Map::getAdjacentTile(QObject* pb){
     QPushButton* p = qobject_cast<QPushButton*>(pb);
     QVector<QPair<int,int>> vertexAdj;
     for(int i=0;i<vertexs.size();i++){
         if(abs(vertexs[i]->x() - (p->x()+60) ) < 85 && abs(vertexs[i]->y() - (p->y()+70) ) < 85 ){
             vertexAdj.append(qMakePair(vertexs[i]->x(),vertexs[i]->y()));
             qDebug() << vertexs[i] ;
         }
     }
     return vertexAdj;
